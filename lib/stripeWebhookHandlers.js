...
export async function handleCheckoutCompleted(session, supabase) {
  // Dynamic checkout: metadata.supabase_user_id and client_reference_id. Legacy: metadata.userId.
  const rawRef =
    session.metadata?.supabase_user_id ?? session.metadata?.userId ?? session.client_reference_id;
  ...
  logWebhook('checkout.session.completed', {
    ...
    metadata_supabase_user_id: session.metadata?.supabase_user_id ?? '(none)',
  ...
  if (!userId) {
    logError('Webhook checkout skipped: no userId in session', null, {
      hint: 'Checkout session must set client_reference_id and metadata.supabase_user_id. Check Stripe Dashboard -> Webhooks -> event payload.',
    });
