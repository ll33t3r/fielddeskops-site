...
export default function LoginPage() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (user && session) {
        ...
        localStorage.setItem('supabase_user', 'true');
        localStorage.setItem('subscription_tier', 'pro_trial'); // Default to pro for now
        localStorage.setItem('credits_remaining', '9999');
  ...
  const handleTrialSignup = () => {
    // Use window.location to avoid useSearchParams
    window.location.href = '/auth/signup?plan=pro_trial';
  };
