...
import { signUpNewUser, signInUser } from '../../../lib/supabase/utils';
...
export default function SignUpPage() {
  ...
  const handleSignUp = async (e) => {
    ...
    try {
      ...
      const { user, session } = await signUpNewUser(
        formData.email,
        formData.password,
        formData.fullName,
        formData.company,
      ...
      if (user) {
        // Check if we got a session (user is logged in automatically)
        if (session) {
          // User is logged in immediately (email confirmation might be disabled)
          console.log('Signup successful, user logged in:', user.email);
          
          // Store basic info in localStorage for fallback
