...
export default function SignOff() {
  ...
  // --- RENDER ---
  return (
    <div className="min-h-screen bg-background text-foreground font-inter pb-32 print:bg-white print:text-black">
      <style jsx global>{`
        @media print {
      ...
      <main className="max-w-4xl mx-auto px-6 pt-4">
        ...
        {/* TABS */}
        <div className="flex bg-industrial-bg p-1 rounded-xl mb-6 border border-industrial-border no-print">
            <button onClick={() => setActiveTab("NEW")} className={`flex-1 py-3 rounded-lg font-bold font-oswald tracking-wide transition-all ${activeTab === "NEW" ? "bg-[#FF6700] text-black shadow-lg" : "text-industrial-muted hover:text-foreground"}`}>
                NEW DOCUMENT
            </button>
            <button onClick={() => setActiveTab("HISTORY")} className={`flex-1 py-3 rounded-lg font-bold font-oswald tracking-wide transition-all ${activeTab === "HISTORY" ? "bg-white text-black shadow-lg" : "text-industrial-muted hover:text-foreground"}`}>
                HISTORY
            </button>
        </div>
        ...
        {activeTab === "NEW" && (
            <div id="print-area" className="animate-in fade-in slide-in-from-left-4">
                ...
                <div className="bg-white text-black p-8 rounded-xl shadow-2xl relative min-h-[80vh] flex flex-col justify-between print:shadow-none print:rounded-none">
                                ...
                                {isSigned ? (
                                    ...
                                    <div ref={containerRef} className="relative border-2 border-dashed border-gray-400 rounded bg-white hover:border-[#FF6700] transition">
                                        <SignatureCanvas 
                                            ref={sigPad}
                                            penColor="black"
                                            clearOnResize={false} // <--- THE BUG FIX
        ...
        {activeTab === "HISTORY" && (
            <div className="animate-in fade-in slide-in-from-right-4 space-y-4 pb-20">
                {contracts.length === 0 ? (
                    <div className="text-center text-industrial-muted py-10">No documents saved yet.</div>
                ) : contracts.map(c => (
                    <div key={c.id} className="glass-panel p-4 rounded-xl flex justify-between items-center group">
                        <div className="flex items-center gap-4">
                            <div className="w-12 h-12 bg-white/5 rounded-lg flex items-center justify-center border border-industrial-border">
                                <FileText className="text-[#FF6700]" size={24}/>
                            </div>
                            <div>
                                <h3 className="font-bold text-lg text-foreground">{c.project_name}</h3>
                                <p className="text-xs text-industrial-muted flex items-center gap-2">
                        ...
                        <div className="flex gap-2 opacity-100 sm:opacity-50 sm:group-hover:opacity-100 transition">
                            <button onClick={() => {
                                ...
                                setIsSigned(true);
                                setActiveTab("NEW");
                            }} className="p-2 bg-white/10 rounded hover:bg-white hover:text-black transition text-foreground">
                                <Printer size={18}/>
                            </button>
