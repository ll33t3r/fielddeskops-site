...
export default function SignOff() {
  ...
  const loadAllData = async () => {
    ...
    // Merge DB templates with defaults, avoiding duplicates by ID
  ...
  const togglePin = async (templateId) => {
      ...
      if (!target.is_pinned && pinnedCount >= 5) return alert("Max 5 templates allowed on dashboard.");
  ...
  const saveContract = async () => {
    ...
    if (!clientName || !contractBody || !finalProjectName) return alert("Missing details.");
    setSaving(true);
    const { data: { user } } = await supabase.auth.getUser();
    ...
    if (newDoc) {
        ...
        setIsSigned(true);
        showToast("Agreement Locked", "success");
    }
    setSaving(false);
  };
      ...
      <div className="flex items-center justify-between px-6 pt-4 mb-4 no-print">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs opacity-60 font-bold tracking-widest">CONTRACTS</p>
          </div>
        </div>
        <button onClick={() => setShowSettings(true)} className="industrial-card p-3 rounded-xl text-[#FF6700] border border-[var(--border-color)] shadow-md">
          <Menu size={24} />
        </button>
      </div>
      ...
      <main className="max-w-4xl mx-auto px-6">
        ...
        <div className="bg-white text-black p-8 rounded-xl shadow-2xl relative min-h-[75vh] flex flex-col border border-gray-300">
            <div className="flex justify-between items-start mb-8 border-b-2 border-black pb-4">
                ...
                {(isSigned || hasSigned) && <div className="border-4 border-black text-black font-black px-4 py-1 rounded uppercase rotate-[-12deg] opacity-70 text-2xl font-oswald flex items-center gap-2"><Lock size={20}/> LOCKED</div>}
            </div>
            ...
            <div className="space-y-6 flex-1">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="border-b border-gray-200 pb-2 relative">
                        ...
                        {isSigned || hasSigned ? (
                           <div className="flex items-center justify-between"><p className="font-bold text-lg uppercase">{clientName}</p><Lock size={12} className="text-gray-300"/></div>
                        ) : (
                    ...
                    <div className="border-b border-gray-200 pb-2 relative">
                        ...
                        {isSigned || hasSigned ? (
                           <div className="flex items-center justify-between"><p className="font-bold text-lg uppercase">{contractorName}</p><Lock size={12} className="text-gray-300"/></div>
                        ) : (
                           <input value={contractorName} onChange={e => setContractorName(e.target.value)} placeholder="YOUR COMPANY NAME..." className="w-full font-bold text-lg outline-none bg-transparent uppercase" />
                        )}
                    </div>
                </div>
                ...
                <div className="relative">
                    ...
                    {isSigned || hasSigned ? (
                        ...
                        <textarea value={contractBody} onChange={e => setContractBody(e.target.value)} placeholder="Type scope here..." className="w-full h-80 font-mono text-sm leading-relaxed bg-zinc-50/50 p-4 border border-dashed border-gray-300 rounded-lg outline-none focus:border-[#FF6700] transition" />
                    )}
                </div>
            </div>
        ...
        <div className="mt-8 flex gap-3 no-print">
            {!isSigned ? (
                ...
                <button onClick={() => { setIsSigned(false); setHasSigned(false); setClientName(""); setContractBody(""); }} className="w-full bg-zinc-800 text-white font-black py-5 rounded-2xl shadow-xl">NEW DOCUMENT</button>
            )}
        </div>
      </main>
                              ...
                              {templates.map(t => (
                                  <div key={t.id} className={`industrial-card p-4 rounded-xl flex justify-between items-center ${t.is_pinned ? 'border-[#FF6700]' : 'border-[var(--border-color)]'}`}>
                                      ...
                                      <div className="flex gap-2">
                                          ...
                                          <button onClick={() => { applySmartTemplate(t.body); setShowSettings(false); }} className="p-2 bg-zinc-800 text-[#FF6700] rounded-lg"><Copy size={14}/></button>
                                      </div>
                                  </div>
                              ))}
                          </div>
