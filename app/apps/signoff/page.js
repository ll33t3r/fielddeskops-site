...
import { 
  ...
  FileText, Calendar, User, Trash2, CheckCircle2, Loader2, X, Lock, ArrowLeft 
} from 'lucide-react';
...
export default function SignOff() {
  ...
  // --- RENDER ---
  return (
    <div className="flex flex-col p-4 max-w-4xl mx-auto space-y-6">
      <style jsx global>{`
        @media print {
      ...
      <div className="flex items-center gap-4 no-print">
        ...
        <div>
          ...
          <p className="text-xs text-gray-400 font-bold tracking-widest opacity-60">CONTRACTS & WAIVERS</p>
        </div>
      </div>

      {/* TABS (Industrial Card) */}
      <div className="flex industrial-card p-1 rounded-xl mb-6 no-print">
          <button onClick={() => setActiveTab('NEW')} className={`flex-1 py-3 rounded-lg font-bold font-oswald tracking-wide transition-all ${activeTab === 'NEW' ? 'bg-[#FF6700] text-black shadow-lg' : 'text-[var(--text-sub)] hover:text-[var(--text-main)]'}`}>
              NEW DOCUMENT
          </button>
          <button onClick={() => setActiveTab('HISTORY')} className={`flex-1 py-3 rounded-lg font-bold font-oswald tracking-wide transition-all ${activeTab === 'HISTORY' ? 'bg-[var(--text-main)] text-[var(--bg-main)] shadow-lg' : 'text-[var(--text-sub)] hover:text-[var(--text-main)]'}`}>
              HISTORY
          </button>
      </div>
      ...
      {activeTab === 'NEW' && (
          <div id="print-area" className="animate-in fade-in slide-in-from-left-4">
              ...
              {/* PAPER CONTAINER (Always White for Document feel) */}
              <div className="bg-white text-black p-8 rounded-xl shadow-2xl relative min-h-[80vh] flex flex-col justify-between print:shadow-none print:rounded-none border border-gray-300">
                  ...
                  <div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                          <div>
                              ...
                              {isSigned || hasSigned ? (
                                  ...
                                  <input 
                                      className="w-full bg-white border-b-2 border-gray-300 p-2 font-bold focus:border-[#FF6700] outline-none transition text-black" 
                      ...
                      {!isSigned && !hasSigned && (
                          <div className="flex gap-2 mb-4 overflow-x-auto no-print pb-2">
                              {TEMPLATES.map(t => (
                                  <button key={t.label} onClick={() => applyTemplate(t.text)} className="whitespace-nowrap px-3 py-1 bg-white border border-gray-300 rounded text-xs font-bold hover:bg-gray-100 transition text-black">
                                      + {t.label}
                                  </button>
                              ))}
                          </div>
                      ...
                      <div className="mb-8 relative">
                          ...
                          {isSigned || hasSigned ? (
                              <div className="whitespace-pre-wrap font-mono text-sm bg-white p-4 border rounded relative text-black">
                                  {contractBody}
                                  {hasSigned && !isSigned && (
                              ...
                              <textarea 
                                  className="w-full h-64 bg-white border border-gray-300 rounded p-4 font-mono text-sm focus:border-[#FF6700] outline-none resize-none shadow-inner text-black"
              ...
              <div className="mt-6 flex gap-3 no-print">
                  {!isSigned ? (
                      ...
                      <div className="flex-1 flex gap-3">
                          <button onClick={printContract} className="flex-1 bg-white text-black font-bold py-4 rounded-xl shadow-lg hover:bg-gray-200 transition flex items-center justify-center gap-2 border border-gray-300">
      ...
      {activeTab === 'HISTORY' && (
          <div className="animate-in fade-in slide-in-from-right-4 space-y-4 pb-20">
              {contracts.length === 0 ? (
                  <div className="text-center text-[var(--text-sub)] py-10">No documents saved yet.</div>
              ) : contracts.map(c => (
                  <div key={c.id} className="industrial-card p-4 rounded-xl flex justify-between items-center group">
                      <div className="flex items-center gap-4">
                          <div className="w-12 h-12 bg-white/5 rounded-lg flex items-center justify-center border border-[var(--border-color)]">
                              <FileText className="text-[#FF6700]" size={24}/>
                          </div>
                          <div>
                              <h3 className="font-bold text-lg text-[var(--text-main)]">{c.project_name}</h3>
                      ...
                      <div className="flex gap-2 opacity-100 sm:opacity-50 sm:group-hover:opacity-100 transition">
                          <button onClick={() => {
                              ...
                              setIsSigned(true);
                              setActiveTab('NEW');
                          }} className="p-2 bg-white/10 rounded hover:bg-white hover:text-black transition text-[var(--text-main)]">
                              <Printer size={18}/>
                          </button>
