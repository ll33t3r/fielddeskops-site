...
export default function SignOff() {
  ...
  const loadAllData = async () => {
    ...
    // STANDARDIZED MULTI-USE TEMPLATES
    const defaults = [
        { label: "STANDARD SERVICE", body: "I, [Client Name], authorize work to proceed. \n\nSCOPE: \nPAYMENT: Due upon completion unless otherwise specified. \nWARRANTY: 30-day labor warranty included." },
        { label: "LIABILITY WAIVER", body: "Customer acknowledges that [Company] is not responsible for damages resulting from pre-existing conditions, corrosion, or structural weaknesses discovered during work." },
        { label: "PLUMBING: DRAIN CLEAN", body: "Work authorized: Main line / secondary drain cleaning. \n\nDISCLAIMER: We are not responsible for broken cables in collapsed or root-infested lines. If excavation is required, a separate quote will be provided." },
        { label: "HVAC: DIAGNOSTIC", body: "Diagnostic service performed. Customer acknowledges system condition. \n\nRECOMMENDATION: \nESTIMATED REPAIR: $" },
        { label: "CHANGE ORDER", body: "The following additional work is required beyond the original scope: \n\nADDITIONAL COST: $ \nNEW COMPLETION DATE: " }
    ];
  ...
  const saveNewTemplate = async () => {
      ...
      if (data) {
          ...
          showToast("New Template Added", "success");
      }
  };
  ...
  const saveContract = async () => {
    ...
    if (!clientName || !contractBody || !finalProjectName) return alert("Fill out all details first.");
    setSaving(true);
    const { data: { user } } = await supabase.auth.getUser();
      ...
      <main className="max-w-4xl mx-auto px-6">
        ...
        <div className="relative mb-6">
            ...
            {isDropdownOpen && !isCustomJob && (
                <div className="absolute top-full left-0 w-full mt-2 bg-[var(--bg-main)] border-2 border-[var(--border-color)] rounded-xl z-50 overflow-hidden">
        ...
        <div className="bg-white text-black p-8 rounded-xl shadow-2xl relative min-h-[70vh] flex flex-col border border-gray-300">
            ...
            <div className="space-y-6 flex-1">
                ...
                <div className="relative">
                    ...
                    {isSigned || hasSigned ? <div className="whitespace-pre-wrap font-mono text-sm leading-relaxed">{contractBody}</div> : <textarea value={contractBody} onChange={e => setContractBody(e.target.value)} placeholder="Type terms here..." className="w-full h-80 font-mono text-sm leading-relaxed bg-zinc-50 p-4 border border-dashed border-gray-300 rounded-lg outline-none focus:border-[#FF6700] transition" />}
                </div>
            </div>
                  ...
                  <div className="overflow-y-auto max-h-[70vh] space-y-4">
                      {settingsTab === "HISTORY" ? (
                          ...
                          <div className="space-y-4">
                              <button onClick={() => setEditingTemplate({label: "", body: ""})} className="w-full py-4 border-2 border-dashed border-[#FF6700] text-[#FF6700] font-black rounded-xl text-xs hover:bg-[#FF6700]/5 transition">+ CREATE CUSTOM TEMPLATE</button>
                              
                              {editingTemplate && (
                                  <div className="industrial-card p-4 rounded-xl space-y-4 border-[#FF6700]">
                                      <input placeholder="TEMPLATE TITLE (E.G. PLUMBING)" value={editingTemplate.label} onChange={e => setEditingTemplate({...editingTemplate, label: e.target.value.toUpperCase()})} className="w-full bg-transparent border-b border-[var(--border-color)] pb-2 font-black text-sm outline-none" />
