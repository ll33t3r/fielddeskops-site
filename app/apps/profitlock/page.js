...
export default function ProfitLock() {
  ...
  useEffect(() => {
    localStorage.setItem('profitlock_data', JSON.stringify({
      bidHistory,
      hourlyRate,
      markupPercent
    }));
  ...
  // ============ STRICTER PROFIT BAR LOGIC (V5 Psychology Engine) ============
  const getProfitMeterInfo = (margin) => {
    // Visual width: scale by 1.6 so 60% fills the whole container
    // This makes thin margins look painfully small
  ...
  const saveBid = () => {
    ...
    if (materials === 0 && hours === 0) {
      showToast('Add materials or labor hours', 'warning');
      return;
    }
    ...
    <div className="min-h-screen w-full max-w-2xl mx-auto p-4" style={{ backgroundColor: '#1a1a1a' }}>
      ...
      <div className="mb-8 pt-4">
        <div className="flex items-center gap-2 mb-2">
          ...
          <h1 className="text-3xl md:text-4xl font-bold" style={{ fontFamily: "'Oswald', sans-serif", letterSpacing: '0.03em', color: '#f5f5f5' }}>PROFITLOCK</h1>
          <span className="text-xs md:text-sm" style={{ color: '#888', marginLeft: '8px' }}>V5</span>
        </div>
        <p style={{ color: '#888' }}>Real-time Bid Calculator with Stricter Psychology</p>
      </div>
      ...
      <div className="p-4 md:p-6 rounded-lg mb-6" style={{ backgroundColor: '#262626', border: '1px solid #404040' }}>
        ...
        {/* PROFIT METER - STRICTER V5 PSYCHOLOGY ENGINE */}
        <div className="mb-6">
          <div style={{
            width: '100%',
            height: '40px',
      ...
      {toast && (
        <div
          style={{
            ...
            bottom: '20px',
            right: '20px',
            backgroundColor: toast.type === 'success' ? '#22c55e' : toast.type === 'error' ? '#ff4444' : '#ffaa00',
            color: '#1a1a1a',
            padding: '16px 24px',
            borderRadius: '8px',
            fontWeight: '700',
