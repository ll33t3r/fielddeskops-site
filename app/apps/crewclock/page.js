...
import {
  ...
  Loader2, Navigation, AlertTriangle, Smartphone
} from "lucide-react";
...
export default function CrewClock() {
  ...
  const [gpsStatus, setGpsStatus] = "WAITING");
  ...
  const handleClockOut = async () => {
    ...
    try {
      ...
      try {
        ...
        console.log("End GPS failed, proceeding...");
      }
  ...
  return (
    <div className="min-h-screen bg-industrial-bg text-white font-inter pb-20">
      ...
      <main className="max-w-xl mx-auto px-6 space-y-6">
        {/* BIG CLOCK */}
        <div
          className={`glass-panel rounded-2xl p-8 border-2 transition-all shadow-2xl relative overflow-hidden ${
            activeSession ? "border-green-500" : "border-industrial-border"
          }`}
        >
          {activeSession ? (
            <>
              <p className="text-sm font-bold text-green-500 uppercase tracking-widest mb-2 animate-pulse">
                ● On The Clock
              </p>
              ...
              <div className="flex justify-center mb-6">
                {activeSession.gps_verified ? (
                  <a
                    href={`http://maps.google.com/?q=${activeSession.start_lat},${activeSession.start_lng}`}
                    target="_blank"
                    className="text-xs bg-green-500/20 text-green-500 border border-green-500/50 px-3 py-1 rounded-full flex items-center gap-1 hover:bg-green-500/30"
                  >
              ...
              <button
                ...
                disabled={processing}
                className="w-full bg-red-600 hover:bg-red-700 text-white font-bold py-5 rounded-xl flex items-center justify-center gap-3 transition active:scale-95 shadow-[0_0_20px_rgba(220,38,38,0.4)]"
              >
            ...
            <>
              ...
              <div className="mb-6 bg-[#1a1a1a] rounded-lg p-2 border border-industrial-border flex items-center gap-2">
                <Briefcase size={18} className="text-industrial-orange ml-2" />
                <select
                  value={selectedJob}
              ...
              <button
                ...
                disabled={processing}
                className="w-full bg-green-600 hover:bg-green-700 text-black font-bold py-5 rounded-xl flex items-center justify-center gap-3 transition active:scale-95 shadow-[0_0_20px_rgba(34,197,94,0.4)] text-xl"
              >
        ...
        <div>
          ...
          <div className="space-y-3">
            {history.map((log) => (
              <div
                key={log.id}
                className="glass-panel rounded-xl p-4 flex justify-between items-center group hover:border-industrial-orange transition"
              >
                <div>
                  ...
                  <div className="text-xs text-gray-500 flex items-center gap-2 mt-1">
                    ...
                    {log.start_lat ? (
                      ...
                      <span className="text-red-500 flex items-center gap-1">
                        <Smartphone size={10} /> No Loc
                      </span>
                    )}
                  </div>
                </div>
                <div className="text-right">
                  <span className="block font-oswald text-xl text-industrial-orange">{log.duration_minutes}m</span>
