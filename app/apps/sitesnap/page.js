...
import { 
  Camera, Trash2, Printer, ArrowLeft, 
...
export default function SiteSnap() {
  ...
  // 5. SMART SHARE HANDLER (Fixed)
  const handleShare = async () => {
    // A. Build a Clean Summary
    const date = new Date().toLocaleDateString();
    let text = `FIELD REPORT - ${date}\n\n`;
    
    // Group photos by Job Name for cleaner text
    const grouped = {};
    photos.forEach(p => {
    ...
    Object.keys(grouped).forEach(job => {
        ...
        grouped[job].forEach(p => {
            ...
            // Add the DIRECT link (Publicly accessible)
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          ...
          text: text,
          // Removed 'url' so it doesn't default to the login page
        });
      } catch (err) {
        ...
        <div className="bg-[#262626] border border-[#404040] rounded-xl p-4 shadow-xl mb-8 no-print">
            <div className="mb-4">
                ...
                {(selectedJob === "custom" || selectedJob === "") && (
                    <input type="text" placeholder="Or type Job Name..." value={customJob} onChange={(e)=>setCustomJob(e.target.value)} className="w-full bg-[#1a1a1a] border border-[#404040] rounded-lg p-3 text-white focus:border-[#FF6700] outline-none" />
                )}
            </div>
            ...
            {!preview ? (
                <div className="grid grid-cols-2 gap-3 h-32 mb-4">
                    <button onClick={()=>cameraInputRef.current.click()} className="border-2 border-dashed border-[#404040] rounded-xl flex flex-col items-center justify-center text-gray-400 hover:border-[#FF6700] hover:text-[#FF6700] bg-[#1a1a1a] transition">
                        <Camera size={32} className="mb-2" />
                        <span className="font-oswald text-sm">TAKE PHOTO</span>
                    </button>
                    <button onClick={()=>fileInputRef.current.click()} className="border-2 border-dashed border-[#404040] rounded-xl flex flex-col items-center justify-center text-gray-400 hover:border-[#FF6700] hover:text-[#FF6700] bg-[#1a1a1a] transition">
                ...
                <div className="relative rounded-xl overflow-hidden border border-[#FF6700] mb-4 bg-black flex items-center justify-center h-48">
                    {fileType === 'pdf' ? (
                        <div className="text-center text-red-500"><FileDigit size={48} className="mx-auto mb-2" /><span className="font-bold">PDF DOCUMENT SELECTED</span></div>
                    ) : (
            ...
            <input type="text" placeholder="Notes (e.g. Receipt for supplies)" value={notes} onChange={(e)=>setNotes(e.target.value)} className="w-full bg-[#1a1a1a] border border-[#404040] rounded-lg p-3 text-white focus:border-[#FF6700] outline-none mb-3" />
            <button onClick={savePhoto} disabled={uploading} className="w-full bg-[#FF6700] text-black font-oswald font-bold text-lg py-4 rounded-lg hover:bg-[#cc5200] transition flex items-center justify-center gap-2">
