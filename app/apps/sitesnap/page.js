  ...
  return (
    <div className={`min-h-screen bg-background text-foreground font-inter pb-32 ${showReportModal || fullScreenImage ? "overflow-hidden h-screen" : ""}`}>
      ...
      <div className="flex items-center justify-between px-6 pt-4 mb-4">
        ...
        {/* HAMBURGER BUTTON - Added border for visibility in Light Mode */}
        ...
        <div className="relative mb-6" ref={dropdownRef}>
          {isCustomizing ? (
            <div className="flex items-center gap-2 bg-zinc-100 dark:bg-zinc-900 border border-[#FF6700] p-4 rounded-xl shadow-lg transition-all">
              ...
              <button onClick={() => setIsCustomizing(false)} className="text-zinc-500"><X size={22}/></button>
            </div>
          ) : (
            <>
              <button onClick={() => { vibrate(5); setIsDropdownOpen(!isDropdownOpen); }} className="w-full flex items-center justify-between bg-card border border-zinc-200 dark:border-white/5 p-4 rounded-xl hover:border-[#FF6700] shadow-md transition-all">
        ...
        <div className="bg-card rounded-2xl p-5 mb-8 border border-zinc-200 dark:border-white/5 shadow-xl">
            <div className="grid grid-cols-3 gap-2 mb-4">
                {["BEFORE", "AFTER", "DOCS"].map(t => (
                    <button key={t} onClick={() => { vibrate(); setTag(t); }} className={`py-3 rounded-lg font-bold text-xs border transition-all ${tag === t ? "bg-[#FF6700] text-black border-[#FF6700]" : "bg-zinc-100 dark:bg-black/20 border-zinc-200 dark:border-white/5 text-zinc-500"}`}>
                        {t}
                    </button>
                ))}
            </div>
            {!preview ? (
                <div className="grid grid-cols-2 gap-3 h-28 mb-4">
                    {/* ADDED BORDERS HERE FOR LIGHT MODE VISIBILITY */}
                    <button onClick={() => cameraInputRef.current.click()} className="border-2 border-dashed border-zinc-300 dark:border-white/10 rounded-xl flex flex-col items-center justify-center text-zinc-500 hover:border-[#FF6700] hover:bg-[#FF6700]/5 transition">
                      <Camera size={28} className="mb-1" />
                      <span className="font-oswald text-[10px] uppercase font-bold tracking-widest">SNAP</span>
                    </button>
                    <button onClick={() => fileInputRef.current.click()} className="border-2 border-dashed border-zinc-300 dark:border-white/10 rounded-xl flex flex-col items-center justify-center text-zinc-500 hover:border-[#FF6700] hover:bg-[#FF6700]/5 transition">
            ...
            {/* NOTES INPUT - UPDATED FOR LIGHT MODE */}
            <input 
              placeholder="Add Notes (Optional)..." 
              value={notes} 
              onChange={(e) => setNotes(e.target.value)} 
              className="bg-zinc-100 dark:bg-zinc-800 border border-zinc-300 dark:border-white/5 rounded-xl p-4 w-full mb-4 outline-none focus:border-[#FF6700] text-foreground" 
            />
      ...
      {showArchive && (
          <div className="fixed inset-0 z-[60] animate-in fade-in">
              ...
              <div className="absolute right-0 top-0 bottom-0 w-80 bg-background border-l border-zinc-200 dark:border-white/5 p-6 animate-in slide-in-from-right">
                  <div className="flex justify-between items-center mb-8 pb-4 border-b border-zinc-200 dark:border-white/5">
                    <h2 className="text-xl font-oswald font-bold text-[#FF6700]">JOB ARCHIVE</h2>
                    <button onClick={() => setShowArchive(false)} className="text-zinc-400"><X /></button>
                  </div>
                  <div className="space-y-4 overflow-y-auto max-h-[85vh]">
                      {allJobs.map(job => (
                          <button key={job} onClick={() => { handleJobSelect(job); setShowArchive(false); }} className={`w-full text-left p-4 rounded-xl border flex flex-col gap-1.5 transition-all ${selectedJob === job ? "bg-[#FF6700]/10 border-[#FF6700]" : "bg-zinc-50 dark:bg-white/5 border-zinc-200 dark:border-white/5 hover:bg-zinc-100 dark:hover:bg-white/10"}`}>
                              <div className="flex items-center gap-2 text-[10px] text-zinc-400 dark:text-zinc-500 font-bold uppercase"><Clock size={12} />{new Date(jobMetadata[job]).toLocaleDateString()}</div>
